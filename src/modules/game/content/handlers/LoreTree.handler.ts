...
export type { LoreTreeNode, LoreNodeRef, LoreNodeDetail, FocusedLoreTreeNode };

/**
 * Handler for Lore Tree traversal and retrieval operations
 */
export default class LoreTreeHandler {
  /**
   * Fetch focused context for a specific lore node with lineage and descendants
   * @param nodeId - The node ID to focus on
   * @param descendantDepth - How many levels of descendants to include (default: 2)
   * @returns Focused lore context with lineage, tree, and metadata
   */
   ...
   */
  async fetchTreeForSetting(settingKey: string): Promise<LoreTreeNode[]> {
  ...
  /**
   * Fetch children for a specific lore node
   * @param parentId - The parent node ID
   * @returns Array of child lore tree nodes
   */
  ...
  /**
   * Fetch lore node by setting key and unique key with full context
   * @param settingKey - The setting key
   * @param key - The unique key within the setting
   * @returns Lore node with parent, ancestors, and children context
   */
